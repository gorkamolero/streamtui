...
func TMDBAPIKey() string {
	...
	return RandomTMDBPoolKey()
}

func RandomTMDBPoolKey() string {
	if len(tmdbKeyPool) == 0 {
		return ""
	}
	index := time.Now().UnixNano() % int64(len(tmdbKeyPool))
	return tmdbKeyPool[index]
}

func IsTMDBPoolKey(key string) bool {
	for _, candidate := range tmdbKeyPool {
		if key == candidate {
			return true
		}
	}
